...
class _PointPageState extends State<PointPage> {
  ...
  String textInput = '';
  late double DEFAULT_VALUE_TOTALWINLOSS = 0.000;
  late double? totalWinLoss = DEFAULT_VALUE_TOTALWINLOSS;

  //BOX STORAGE
  runApi(
      {required String? number,
      required MyGetXController controller,
      required bool? isSaveLocal}) async {
      ...
      if ((value.length) > 0) {
        service
          ...
          if (model.status == true) {
            ...
            ScaffoldMessenger.of(context)
                .showSnackBar(mysnackBar('No data found'));
          }
        });
      } else {
        ScaffoldMessenger.of(context).showSnackBar(mysnackBar('No data found'));
      }
    });
  }
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ...
      if (myCtl.hasChangeVoucherStatus.value == true) {
        runApi(
            ...
            isSaveLocal: true);
        // runWinLossApi(myCtl.valueSearchInput.value);
      }
    });
  }

  void runWinLossApi(String number) {
    debugPrint("runWinLossApi with number : $number");
    //make date from today to tomorrow
    // DateTime tomorrow = DateTime.now().add(Duration(days: 1));
    // //RUN WIN/LOSS APIs
    service.machineWinLossDate( date: format.formatDate(DateTime.now().add(const Duration(days: 0))), number: number).then((v) {
      double total = 0.0; // Initialize the totalWinLoss variable
      for (var i = 0; i < v.list.length; i++) {
        //  debugPrint('allvalue: ${v.list[i].winLoss}');
        total += v.list[i].winLoss; // Accumulate the winLoss values
        debugPrint('totalvalue: $total');
      }
      setState(() {
        totalWinLoss = total; // Update the state with the accumulated totalWinLoss
        listPlaying = v.list;
      });
      // debugPrint('Total WinLoss: $totalWinLoss');
      myCtl.updateUserData(newWinLoss:  totalWinLoss.toString(),newDetailMCPlaying: listPlaying);
    });
  }
                ...
                children: [
                  IconButton(
                      ...
                      icon: GetBuilder<MyGetXController>(
                        builder: (controller) => Tooltip(
                          message: "Frame Update",
                          child: Icon(controller.isShowListSearch.value == false
                              ? Icons.filter_list
                        ...
                        : TextField(
                            ...
                            onSubmitted: (value) {
                              runApi(
                                  ...
                                  isSaveLocal: true);
                              // runWinLossApi(value);
                              debugPrint('winloss  pint    api : $value');
                              runWinLossApi(value);
                            },
                          ))),
        body: GetBuilder<MyGetXController>(
          builder: (myCtl) => Stack(
            children: [
              Container(
                  ...
                  child: detectPlatform() == false
                      ? myCtl.valueSearchInput.value == ''
                          ...
                          : bodyPoint(
                              listPlaying:  myCtl.detailMCPlaying,
                              totalWinLoss:totalWinLoss! == DEFAULT_VALUE_TOTALWINLOSS ? double.parse(myCtl.winLoss.value) : totalWinLoss!,
                              onTapListSeach: () {
                                runApi(
                                    controller: myCtl,
                                    number: myCtl.savedNumber.value,
                                    isSaveLocal: false);
                                //run api win/loss too
                      ...
                      : bodyPoint(
                          ...
                          totalWinLoss: double.parse(myCtl.winLoss.value),
                          onTapListSeach: () {
                            runApi(
                                controller: myCtl,
                                number: myCtl.savedNumber.value,
              ...
              // SEARCH VIEW
              Visibility(
                visible: myCtl.isShowListSearch.value,
                child: Positioned(
                  right: 0,
                  top: 0,
                  child: Opacity(
