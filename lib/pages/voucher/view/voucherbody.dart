...
class VoucherPageBody extends StatefulWidget {
  const VoucherPageBody({Key? key}) : super(key: key);

  @override
  State<VoucherPageBody> createState() => _VoucherPageBodyState();
}

class _VoucherPageBodyState extends State<VoucherPageBody> {
  final TextEditingController searchInputController = TextEditingController();
  final MyGetXController  myCtl = Get.put(MyGetXController());
  ...
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      debugPrint('VoucherPageBody valueSearchInput: ${myCtl.valueSearchInput.value}');
    });
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return DefaultTabController(
        ...
        child: GetBuilder<MyGetXController>(
            builder: (controller) => BlocListener<VoucherBloc, VoucherState>(
                    ...
                    builder: (context, state) {
                      return Scaffold(
                        appBar: AppBar(
                            ...
                            title: detectPlatform() == false
                                ? searchTextField(
                                    controller: searchInputController,
                                    hasInitValue: true,
                                    valueInit: controller.searchInputValue,
                                    hint: 'All Vouchers',
                                    onSubmit: (String value) {
                                      if (validateFieldSearch(value, context)) {
                                        ...
                                        debugPrint('value save input voucherpage: ${controller.searchInputValue}');
                                        if (value !=controller.searchInputValue) {
                                          controller.turnOnVouchersStatus();
                                        }
                                      }
                                    })
    ...
    case VoucherStatus.success:
      ...
      return Stack(
        children: [
          ...
          //show total results
          resultRow(context: context!, total: voucherList.length)
        ],
      );
  }
}
